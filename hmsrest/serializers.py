...
from .models import Item, Purchase, DailyMenu
...
class ItemSerializerfortransfer(serializers.ModelSerializer):
    class Meta:
        model = Item
        fields = ()
...
class PurchaseListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Purchase
        fields = ['id', 'item', 'vendor',
                  'quantity', 'purchase_date', 'status']
...
class ItemTransferSerializer(serializers.ModelSerializer):
    class Meta:
        model = ItemTransfer
        fields = ['id', 'request_user', 'grant_user',
                  'items', 'status', 'transfer_date']
        read_only_fields = ['id']
...
class ItemSerializerforRecipe(serializers.ModelSerializer):
    class Meta:
        model = Item
        fields = ('name', 'id', 'category', 'price')
        read_only_fields = ['category', 'price']
...
class DishSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Dish
        fields = ('id', 'name', 'error_margin', 'cost',
                  'theoretical_quantity', 'items', 'recipes', 'price', 'totalprice', 'category')
...
class DailyMenuSerializer(serializers.ModelSerializer):
    # dishes = DishSerliserForDailyMenu(many=True, read_only=True)
...
class DailyMenuSerializer2(serializers.ModelSerializer):
    dishes = DishSerliserForDailyMenu(many=True, read_only=True)
